...
public class ChatUI {
    private final String NO_CHANNEL = "--                    ";

    private ChatClient cc;  // Main client. We use it as "C" from rhe "MCV"
    ...
    private Label lblCurrentChannel = new Label(NO_CHANNEL);  // Current channel
    private Label lblDestination = new Label(NO_CHANNEL);  // User or Channel, where our messages go
    private boolean bDestinationIsChannel = true;
    private boolean bConnected = false;

    public ChatUI(ChatClient cc) {
        this.cc = cc;
        createUI();
    }
    ...
    private void createUI() {
        ...
        txtMessage.addActionListener(e -> onMessageTyped());
        btnConnect.addActionListener(e -> onConnectButtonPressed());
        txtServer.addActionListener(e -> onConnectionParamsChanged());
        txtNick.addActionListener(e -> onConnectionParamsChanged());

        lstChannelList.addActionListener(e -> onChannelSelected());

        lstUserList.addActionListener(e -> onUserSelected());
    ...
    //
    // Main chat area where incoming messages are shown is "lstChannel".
    ...
    //
    // Private message coming from a remote user
    //
