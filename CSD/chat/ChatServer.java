...
public class ChatServer extends UnicastRemoteObject
    ...
    private Hashtable<String, IChatChannel> channels = new Hashtable<>();
    ...
    private void purgeUsers() {
        ...
        for (String key : keys) {
            try {
                users.get(key).getNick(); // Remote invocation to check if stale or alive
            } catch (Exception e) {
    ...
    private void purgeChannels() {
        ...
        for (String key : keys) {
            try {
                channels.get(key).getName(); // Remote invocation to check if stale or alive
            } catch (Exception e) {
